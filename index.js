...
client.on("message", async (msg) => {
    ...
    if (chat.startsWith("cari ")) {
        ...
        const command = `"${__dirname}\\yt-dlp.exe" "ytsearch20:${query}" --print-json --flat-playlist`;
    ...
    // ✅ Auto-reply hanya untuk DM/pribadi
...
async function downloadAndSendVoice(urlOrQuery, msg, manualTitle = null) {
    ...
    const infoCommand = `"${__dirname}\\yt-dlp.exe" -j "${searchQuery}"`;
    const downloadCommand = `"${__dirname}\\yt-dlp.exe" -f bestaudio --extract-audio --audio-format opus --audio-quality 5 --output "${oggPath}" "${searchQuery}" --ffmpeg-location "${__dirname}"`;
